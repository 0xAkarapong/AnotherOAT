  ...
  return (
    <section className="grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
      <Panel className="p-5 sm:p-6">
        ...
        <h2 className="mt-2 font-serif text-2xl text-white">โครงสร้างที่พร้อมไปต่อเป็นงานวิจัยเชิงสังเกต</h2>
      ...
      <Panel className="p-5 sm:p-6">
        ...
        <div className="mt-4 space-y-3">
          {sourcesConfig.providerList.map((provider) => (
            <div
              className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/15 px-4 py-3 text-sm dark:bg-black/25"
              key={provider.id}
            >
              <div>
              ...
              <span
                className={
                  provider.enabled
                    ? "rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-xs text-emerald-100"
                    : "rounded-full border border-amber-400/20 bg-amber-400/10 px-3 py-1 text-xs text-amber-50"
                }
              >
